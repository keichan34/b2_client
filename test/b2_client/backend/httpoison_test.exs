defmodule B2Client.Backend.HTTPoisonTest do
  ...
  setup_all do
    on_exit fn ->
      ...
      Enum.each(files, fn(file) ->
        ...
        contents = Regex.replace(~r/3_#{year}\d{10}_[0-9a-f]{24}_[0-9a-f]{40}_000_(acct|upld)/, contents, "3_#{year}XXXXXXXXXX_XXXXXXXXXXXXXXXXXXXXXXXX_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX_000_\\1")
  ...
  test "download/3 when the file exists" do
    use_cassette "httpoison_download_3_file_exists" do
      ...
      assert {:ok, file} = backend.download(auth, bucket, "hello_there.txt")

      assert file.contents == "hello there"
      assert file.file_id == "4_z6dd33353ffdd65f85e410312_f1172ee1b90ad92b9_d20160502_m061500_c000_v0001007_t0027"
    end
  end
  ...
  test "upload/4 to a Bucket.t" do
    use_cassette "httpoison_upload_4_bucket" do
      ...
      assert {:ok, %File{} = file} = backend.upload(auth, bucket, "hello there", "hello_there.txt")
